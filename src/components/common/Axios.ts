...
import type { AxiosResponse, InternalAxiosRequestConfig } from "axios";// import dayjs from 'dayjs';
     ...
     function (response: AxiosResponse) {
          if (response?.data?.succeeded === false) {
               try {
                    if (
                         response?.config.url ) {
                         const findError = response?.data?.errors.length
                              ? response?.data?.errors[0]
                              : response.data?.result?.loginResultType;
                        //  findError && toastError({ title: i18next.t(`Errors.${findError as ErrorsNotify}`) });
                        if(findError) {
                            toastError(findError , "5")
                        }
...
//      routerPage.navigate(routerPagePath.auth.logout);
// };
